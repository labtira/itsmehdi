...
function Navbar() {
  ...
  return (
    <nav className={`fixed bottom-0 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-500 backdrop-blur-xl bg-gradient-to-b from-black/10 to-black/30 border-2 rounded-3xl border-[#222222] shadow-[0_-8px_32px_rgba(0,0,0,0.12)] w-[calc(100%-30px)] max-w-[400px] my-[15px] `}>
        ...
        <div className="flex items-center justify-around w-full gap-2">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`flex-1 flex items-center justify-center mx-6 rounded-xl font-medium transition-all duration-300 ease-out group focus:outline-none ${location.pathname === link.path ? 'text-white' : 'text-zinc-400 [@media(hover:hover)]:hover:text-white'}`}
